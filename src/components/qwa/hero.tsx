      ...
      <Container className="pb-20 lg:pb-28">
        <div className="grid items-center gap-14 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:gap-16">
          <div className="min-w-0">
            ...
            <h1 className="text-display mt-7 text-[clamp(2.6rem,7.2vw,4.75rem)]">
            ...
            <dl className="mt-12 grid max-w-lg grid-cols-3 gap-6 border-t border-hairline pt-7">
              {[
                ["< 30s", "First response"],
...
function HeroVisual() {
  ...
  React.useEffect(() => {
    ...
    const id = window.setInterval(() => setActive((i) => (i + 1) % channels.length), 2400);
  ...
  return (
    <div className="relative min-w-0">
      ...
      <div className="glass-panel rounded-[1.75rem] p-4 sm:p-6">
          ...
          <ul className="grid gap-2">
            {channels.map((c, i) => (
              <li
                key={c.label}
                className={cn(
                  "flex min-w-0 items-center justify-between gap-2 rounded-xl border px-3 py-2.5 transition-all duration-500",
                  i === active && !reduced
                    ? "border-signal/40 bg-signal-soft"
                    : "border-hairline bg-card/60",
                )}
              >
                <span className="truncate text-xs font-medium sm:text-sm">{c.label}</span>
                <span className="text-data hidden truncate text-[0.65rem] text-muted-foreground sm:inline">
                  {c.detail}
                </span>
              </li>
            ))}
            ...
            >
              {channels.map((_, i) => (
                <path
                  ...
                  strokeDasharray="4 6"
                  className={i === active ? "animate-flow" : undefined}
                />
              ))}
            </svg>
        ...
        <div className="mt-5 rounded-xl border border-hairline bg-paper px-4 py-3">
          <p className="text-data text-[0.7rem] text-muted-foreground">
            <span className="text-foreground">{current.label}</span> signal captured · qualified in
            8s · routed to booking · attributed to campaign
          </p>
        </div>
      </div>
    </div>
